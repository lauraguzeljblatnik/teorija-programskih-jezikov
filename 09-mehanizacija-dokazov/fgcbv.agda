...
rename-c ρ (LET V IN M) = LET rename-c ρ V IN rename-c (extend-renaming ρ) M -- A JE TO OK?
...
subst-c σ (LET V IN M) = LET (subst-c σ V) IN (subst-c (extend-subst σ) M)
...
progress (LET V IN M)  with progress V
... | is-value = {!   !}
... | steps x = {!   !}
